...
BASE="$(cd "$ROOT/../.." && pwd)"
SRC="$BASE/Notes/Motion_Media/Motion_Media"
OUT="$ROOT/slides"
...
rsync -a --delete "$SRC/A4-marp-main/diagrams/" "$OUT/diagrams/"

HTML_FILES=()

for md in "$SRC"/SHDH2368_Motion_Media_week_*_Lecture.md; do
  base="$(basename "$md")"
  num="$(echo "$base" | sed -E 's/.*week_([0-9]+).*/\1/')"
  # Skip test lecture and later (weeks 11+)
  if (( 10#$num >= 11 )); then
    echo "[skip] week $num lecture (no published deck)"
    continue
  fi
...
"$MARP" "$SRC/A4-marp-main/SSHD2368_Motion_Media_All_Tutorials.md" \
  -o "$OUT/tutorials.html" --html --allow-local-files \
  --theme-set "$SRC/A4-marp-main/a4-clean.css" \
  --theme-set "$SRC/A4-marp-main/a4-dark.css" \
  --theme-set "$SRC/A4-marp-main/a4-light.css"
HTML_FILES+=("$OUT/tutorials.html")

# Inline local images/diagrams as base64, then drop external asset folders
...
rm -f "$OUT/week-11.html" "$OUT/week-12.html" "$OUT/week-13.html"

echo "Exported self-contained lecture (weeks 1–10) + tutorial HTML to $OUT"
